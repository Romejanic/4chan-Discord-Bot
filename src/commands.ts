...
const COMMANDS: CommandHandlers = {
        ...
        switch(ctx.options.getSubcommand(true)) {
            ...
            case "help":
                ...
                if(ctx.isServer) {
                    ...
                    let canEdit = channel.permissionsFor(ctx.user).has("ManageGuild");
                    if(canEdit) {
    ...
    "boards": async (ctx) => {
        ...
        const countButtonId = "boards_count";
        ...
        collect.on("end", async () => {
            ...
            data.components = [];
            embed.setFooter({ text: "Page " + format(STRINGS["boards_pages"], currPage+1, pages) });
            await ctx.edit(data);
        });
    },
    ...
    "browse": async (ctx, lib) => {
        ...
        // create buttons
        const buttonIds = ["browse_back","browse_next","browse_up","browse_down","browse_close"];
        let back  = createButton(buttonIds[0], STRINGS["boards_back"]);
        let next  = createButton(buttonIds[1], STRINGS["boards_next"]);
        let up    = createButton(buttonIds[2], STRINGS["browse_up"]);
        let down  = createButton(buttonIds[3], STRINGS["browse_down"]);
        let close = createButton(buttonIds[4], STRINGS["browse_close"], false, true); 
        ...
        const filter = (i: ButtonInteraction) => buttonIds.includes(i.customId);
        const collect = ctx.channel.createMessageComponentCollector({ componentType: ComponentType.Button, message, filter, time: 10 * 60 * 1000 });
    ...
    "config": async(ctx, lib) => {
        ...
        if(!ctx.server.member.permissions.has("ManageGuild")) {
            let embed = new EmbedBuilder()
                .setColor(EMBED_COLOR_ERROR)
...
async function sendPost(post: chan.ChanPost, ctx: CommandContext, lib: Libs): Promise<void> {
    ...
    let data: InteractionEditReplyOptions = { embeds: [embed] };
    ...
    if(removalEnabled) {
        ...
        const collect = ctx.channel.createMessageComponentCollector({ componentType: ComponentType.Button, filter, message, time: removal_time * 1000 });

        collect.on("collect", async (btn) => {
            // check the user clicking the button is the sender
            let channel = btn.channel;
            let isAdmin = ctx.isServer && (btn.member as GuildMember).permissionsIn(channel).has("ManageMessages");
...
export default {
    ...
    warning: async (msg: Message) => {
        ...
        const { channel } = msg;
        if(channel.isSendable() && await startsWithPrefix(msg)) {
            let embed = new EmbedBuilder()
